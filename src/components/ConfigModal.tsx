...
export default function ConfigModal() {
  ...
  useEffect(() => {
    if (state.configModalOpen) {
      ...
      if (current) { setName(current.name); setApiUrl(current.apiUrl); setApiKey(current.apiKey); }
    }
  ...
  function selectProfile(id: string) {
    ...
    if (p) { setName(p.name); setApiUrl(p.apiUrl); setApiKey(p.apiKey); }
  }
  ...
  function handleSave() {
    ...
    if (profiles.find((p) => p.id === selectedId)) {
      updateProfile(selectedId, { name: trimmedName, apiUrl: trimmedUrl, apiKey: trimmedKey });
    } else {
